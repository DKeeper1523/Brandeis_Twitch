...
def createDictGuess(ls_base_name, ls_target_name, toLower = True):
    #init truth dict with '[nan]'
    LS_EMPTY = ['','nan','[nan]']
    dict_truth = {empty:'EMPTY' for empty in LS_EMPTY}
    for name in ls_base_name:
        name = str(name).lower() if toLower else str(name)
        if name not in LS_EMPTY:
            most_likely = mapMostSimilar(name, ls_target_name)
...
def fix_col_with_replace(df, ls_col, ls_truth, setCol2Mode_ = False):
    ...
    for key in dict_guess.keys():
        df.loc[:,ls_col] = df.loc[:,ls_col].replace(key, dict_guess[key])
